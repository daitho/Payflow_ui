...
void main() {
  ...
  test('history forwards year and reads server year options even on an empty page', () async {
    adapter.contentType = 'application/json';
    adapter.bytes = utf8.encode(jsonEncode({
      'items': [], 'beneficiaries': [], 'availableStatuses': [],
      'availableYears': [2026, 2024], 'page': 0, 'size': 20,
      'totalElements': 0, 'totalPages': 0, 'hasNext': false,
      'summary': {'transactionCount': 0, 'sentTotals': []},
    }));
    final response = await api.getHistory(
      beneficiaryId: 'alice', status: 'COMPLETED', year: 2024, page: 0, size: 20);
  ...
  test('statement forwards all three filters and language, without pagination', () async {
    final bytes = await api.getStatement(
      beneficiaryId: 'alice', status: 'COMPLETED', year: 2024, locale: 'fr-FR');
    expect(adapter.request!.path, '/api/v1/transfers/history/export/pdf');
    expect(adapter.request!.queryParameters, {
      'beneficiaryId': 'alice', 'status': 'COMPLETED',
      'locale': 'fr-FR', 'download': true, 'year': 2024,
    });
