...
const alice = BeneficiaryContact(
  id: 'alice',
  fullName: 'Émilie Dupont',
  countryId: 'cm',
  countryName: 'Cameroun',
  countryCode: 'CM',
  operatorId: 'mtn',
  phoneE164: '+237690000000',
  gender: BeneficiaryGender.female,
);
...
void main() {
  test(
    'all alphabet colors are distinct and stable across case and accents',
    () {
      expect(BeneficiaryPalette.colors.toSet().length, 26);
      expect(
        BeneficiaryPalette.forName('Émilie'),
        BeneficiaryPalette.forName('emma'),
      );
      expect(
        BeneficiaryPalette.forName('Maffo'),
        BeneficiaryPalette.forName('maurice'),
      );
  ...
  test('search ignores accents and case', () async {
    ...
    await vm.load();
    vm.search('emilie');
    expect(vm.items.single.id, 'alice');
    vm.search('absent');
    expect(vm.items, isEmpty);
    vm.dispose();
  });
  test(
    'new form is blank and editing loads stored gender and destination',
    () async {
      final service = BeneficiaryService(FakeRepository());
      final blank = BeneficiaryFormViewModel(service);
      await blank.load();
      expect(blank.original, isNull);
      expect(blank.countryId, isNull);
      expect(blank.gender, isNull);
      final edit = BeneficiaryFormViewModel(service, id: 'alice');
      await edit.load();
      expect(edit.original!.fullName, 'Émilie Dupont');
      expect(edit.gender, BeneficiaryGender.female);
      expect(edit.operatorId, 'mtn');
      edit.selectCountry('sn');
      expect(edit.operatorId, isNull);
      expect(edit.operators.single.id, 'wave');
      blank.dispose();
      edit.dispose();
    },
  );
  test(
    'save retains edit ID, normalizes phone and blocks double taps',
    () async {
      final repo = FakeRepository();
      final vm = BeneficiaryFormViewModel(
        BeneficiaryService(repo),
        id: 'alice',
      );
      await vm.load();
      ...
      repo.pending.complete(alice);
      expect(await save, alice);
      vm.dispose();
    },
  );
}
