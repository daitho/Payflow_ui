...
class ActiveSessionsApiService {
  ...
  ActiveSessionsApiService({required Dio dio}) : _dio = dio;
  ...
  Future<List<ActiveSessionDto>> getSessions() async {
    ...
    return data
        .map(
          (dynamic item) =>
              ActiveSessionDto.fromJson(Map<String, dynamic>.from(item as Map)),
        )
        .toList();
  }
  ...
  Future<void> revokeSession({required String sessionId}) async {
    await _dio.delete('${ApiEndpoints.authSessions}/$sessionId');
  }
  ...
  Future<void> logoutOthers() async {
    await _dio.post(ApiEndpoints.authLogoutOthers);
  }
