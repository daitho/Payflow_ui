...
class BeneficiariesViewModel extends ChangeNotifier {
  ...
  int _generation = 0;
  List<BeneficiaryContact> get items => List.unmodifiable(_items.where((r) =>
    beneficiarySearchKey(r.fullName).contains(beneficiarySearchKey(_query))));
  void search(String value) { _query = value; notifyListeners(); }
  Future<void> load() async {
    ...
    try {
      ...
      if (!_disposed && generation == _generation) {
        error = e is BeneficiaryException ? e.failure : BeneficiaryFailure.server;
      }
    } finally {
