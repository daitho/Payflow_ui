...
class BeneficiaryFormViewModel extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      catalog = result; original = contact;
      countryId = contact?.countryId; operatorId = contact?.operatorId; gender = contact?.gender;
    } catch (e) {
      if (!_disposed && generation == _generation) error = e is BeneficiaryException ? e.failure : BeneficiaryFailure.server;
    } finally {
  ...
  void selectCountry(String? value) {
    ...
    countryId = value; operatorId = null; notifyListeners();
  }
  void selectOperator(String? value) { if (!saving) { operatorId = value; notifyListeners(); } }
  void selectGender(BeneficiaryGender? value) { if (!saving) { gender = value; notifyListeners(); } }
  Future<BeneficiaryContact?> save(String fullName, String phone) async {
    ...
    if (countryId == null || operatorId == null ||
      ...
      error = BeneficiaryFailure.invalid; notifyListeners(); return null;
    }
    saving = true; error = null; notifyListeners();
    try {
      return await service.save(BeneficiaryContactInput(fullName: fullName,
        countryId: countryId!, operatorId: operatorId!, phoneE164: phone, gender: gender), id: id);
    } catch (e) {
      if (!_disposed) error = e is BeneficiaryException ? e.failure : BeneficiaryFailure.server;
      return null;
    } finally {
      if (!_disposed) { saving = false; notifyListeners(); }
    }
  }
  @override
