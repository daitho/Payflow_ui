...
class BeneficiaryRepositoryImpl implements BeneficiaryRepository {
  ...
  @override
  Future<List<BeneficiaryContact>> list() => _guard(() async =>
    (await api.list()).map((r) => r.toModel()).toList());
  @override
  Future<BeneficiaryContact> get(String id) => _guard(() async => (await api.get(id)).toModel());
  @override
  Future<BeneficiaryCatalog> catalog() => _guard(() async => (await api.catalog()).toModel());
  @override
  Future<BeneficiaryContact> save(BeneficiaryContactInput input, {String? id}) => _guard(() async =>
    (await api.save({
      'fullName': input.fullName, 'countryId': input.countryId,
      'operatorId': input.operatorId, 'phoneE164': input.phoneE164,
      'gender': switch (input.gender) {
        BeneficiaryGender.male => 'MALE', BeneficiaryGender.female => 'FEMALE', null => null,
      },
    }, id: id)).toModel());
  ...
  Future<T> _guard<T>(Future<T> Function() action) async {
    ...
    } catch (_) { throw const BeneficiaryException(BeneficiaryFailure.server); }
  }
}
