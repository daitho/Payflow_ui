...
class TransferDetailHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final flag = RegExp(r'^[A-Z]{2}$').hasMatch(code)
      ? String.fromCharCodes(code.codeUnits.map((unit) => unit + 127397)) : code;
    ...
    return Column(children: [
      Text(transferDate(context, detail.createdAt),
      ...
      Text(l10n.transferSentTo(amount, detail.beneficiaryName),
        textAlign: TextAlign.center,
        style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w700)),
      const SizedBox(height: 12),
      if (recipientDetails.isNotEmpty)
        Text(recipientDetails, textAlign: TextAlign.center,
          style: const TextStyle(fontWeight: FontWeight.w600)),
      if (detail.providerReference != null) Text(
        '${l10n.transferProviderReference} : ${detail.providerReference}',
        textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFF948C88))),
      const SizedBox(height: 24),
    ]);
  }
}
