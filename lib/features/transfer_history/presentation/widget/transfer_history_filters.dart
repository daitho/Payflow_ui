...
class TransferHistoryFilters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final beneficiaries = {
      for (final item
          in page?.beneficiaries ?? <TransferHistoryBeneficiaryModel>[])
        item.id: item.displayName,
    };
    if (filter.beneficiaryId != null) {
      beneficiaries.putIfAbsent(
        filter.beneficiaryId!,
        () => l10n.transferBeneficiary,
      );
    }
    final statuses = {
      ...?page?.availableStatuses,
    ...
    final statusField = _dropdown(
      ...
      value: filter.status,
      options: {
        for (final status in statuses) status: transferStatus(l10n, status),
      },
