...
abstract final class TransferHistoryMapper {
  static TransferHistoryPageModel page(TransferHistoryResponseDto dto) {
    return TransferHistoryPageModel(
      ...
      beneficiaries: dto.beneficiaries
          .map(
            (item) =>
                TransferHistoryBeneficiaryModel(item.id, item.displayName),
          )
          .toList(),
      availableStatuses: dto.availableStatuses,
      availableYears: dto.availableYears,
      sentTotals: dto.summary.sentTotals
          .map(
            (item) => TransferHistoryCurrencyTotalModel(
              item.currencyCode,
              item.amount,
            ),
          )
          .toList(),
      page: dto.page,
      transactionCount: dto.summary.transactionCount,
      hasNext: dto.hasNext,
    );
  }
  ...
  static TransferDetailModel detail(TransferDetailDto dto) {
    return TransferDetailModel(
      ...
      fee: dto.fee,
      totalChargedAmount: dto.totalChargedAmount,
      appliedRate: dto.appliedRate,
      createdAt: dto.createdAt,
      receivedAt: dto.receivedAt,
      providerReference: dto.providerReference,
      receiptAvailable: dto.receiptAvailable,
      repeatAllowed: dto.repeatAllowed,
      timeline: dto.statusTimeline
          .map((item) => TransferTimelineEntry(item.status, item.occurredAt))
          .toList(),
    );
  }
}
